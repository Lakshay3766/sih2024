...
def speak_digital_text():  
   try:  
      ...
      print(f"Error in text to speech: {e}")  
      logging.error(f"Text to speech error: {e}")  
def write_spoken_text_to_canvas():  
  spoken_text = speech_to_text.start_transcription("English")  
  canvas = pages[0].winfo_children()[0]  
  canvas.create_text(10, 10, text=spoken_text, font=("Itim", 14), anchor=tk.NW)
...
def speech():  
    print("Recognize functionality")  # Placeholder for OCR logic  
...
speech_to_text_button = tk.Button(toolbar_box, text="Speech to Text", command=write_spoken_text_to_canvas)  
